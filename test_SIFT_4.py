...
def resize_image_to_match(img1, img2):
    ...
    img1_resized = cv2.resize(img1, new_size, interpolation=cv2.INTER_AREA)
    img2_resized = cv2.resize(img2, new_size, interpolation=cv2.INTER_AREA)
...
def insert_watermark_sift_region(original_image_path, watermark_image_path, output_image_path, region_size=32, alpha=0.15):
    ...
    return original_image_data, watermarked_image_data, kp
...
# 워터마크 삽입
original_image_data, watermarked_image_data, kp = insert_watermark_sift_region(original_image_path, watermark_image_path, watermarked_image_path)
...
# 워터마크 추출
x_start, y_start, x_end, y_end = 0, 0, original_image_data.shape[1], original_image_data.shape[0]
