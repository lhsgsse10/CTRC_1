...
def extract_surf_keypoints(image):
    image_cv = np.array(image)
    surf = cv2.xfeatures2d.SURF_create(hessianThreshold=400)
    keypoints, descriptors = surf.detectAndCompute(image_cv, None)
    return keypoints, descriptors

def insert_watermark_surf(original_image_path, watermark_image_path, output_image_path, num_keypoints=300, alpha=0.3):
    start_time = time.time()
    
    original_image = Image.open(original_image_path).convert('L')
    watermark_image = Image.open(watermark_image_path).convert('L')
    
    keypoints, _ = extract_surf_keypoints(original_image)
...
def extract_watermark_surf(watermarked_image_path, original_LL, keypoints, watermark_size, output_watermark_path, alpha=0.3):
    start_time = time.time()
...
# 워터마크 삽입
original_LL, selected_keypoints = insert_watermark_surf(original_image_path, watermark_image_path, watermarked_image_path, num_keypoints=300, alpha=0.3)

# 워터마크 추출
extract_watermark_surf(watermarked_image_path, original_LL, selected_keypoints, Image.open(watermark_image_path).size, extracted_watermark_path, alpha=0.3)
